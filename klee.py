...
class GCov:
    ...
    def run(self, target, gcdas, folder_depth=1):
        ...
        for _ in range(folder_depth):
            if "gawk" in str(target) or "sqlite" in str(target):
                pass
            else:
                base = base / '..'
...
class KLEE(SymbolicExecutor):
    ...
    def run(self, target, parameters, **kwargs):
        ...
        # For Homi
        early_testcases = list(output_dir.glob('*early'))
        early_testcases = [tc.absolute() for tc in early_testcases]

        # Restore original directory.
        os.chdir(str(original_path))

        # return testcases, early_testcases
        return testcases
...
class KLEESymTuner(SymTuner):
    ...
    def add(self, target, testcases, parameters=None, evaluation_kwargs=None, 
            early_testcases=None, iters=None, widx=0, rm_cmd=None, homi_target=None, flag=None):
        ...
        super(KLEESymTuner, self).add(target, testcases, parameters=parameters, evaluation_kwargs=evaluation_kwargs,
                                      early_testcases=early_testcases, iters=iters, widx=widx, rm_cmd=rm_cmd, homi_target=homi_target, flag=flag)

        if flag == "featmaker" or flag == "symtuner":
            return self
        else:
            pass

        ##### Combination Version - Skipped !
    ...
    def combination_version_add(self):
        if '-seed-file' not in self.space.keys() and '--seed-file' not in self.space.keys():
            return self
