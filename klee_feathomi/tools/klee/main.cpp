...
namespace {
  ...
  cl::list<std::string>
  KqueryFiles("kquery-file",
              cl::desc(".kquery file for pruning states with seed"),
              cl::cat(SeedingCat));
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!WriteNone) {
    ...
    if (errorMessage || WriteKQueries || true) {
...
int main(int argc, char **argv, char **envp) {
  ...
  for (int i=0; i < argc; i++){
    ...
    if(str.find(".bc")!=std::string::npos){
      ...
      size_t last_slash = pgm.rfind('/');
      if (last_slash != std::string::npos) {
          pgm = pgm.substr(last_slash + 1);
      }

      if (pgm.find("sqlite3") != std::string::npos) {
        pgm = pgm.replace(pgm.find("sqlite3"), 7, "sqlite");
      }
    }

    else if (str.find("-symmode=")!=std::string::npos){
      std::string delimiter = "-symmode=";
      std::string symmode = str.substr(str.find(delimiter) + delimiter.length(), str.length());
      mode = "homi";
      if (symmode.find("mode") != std::string::npos) {
        homiflag = true;
      }

      else if (symmode.find("only") != std::string::npos) {
        homiflag = false;
      }
    }

    else if (str.find("--iterIndex=") != std::string::npos) {
      std::string delimiter = "--iterIndex=";
      iterIndex = str.substr(str.find(delimiter) + delimiter.length(), str.length());
    }
  ...
  // if (!mode.empty()) {
  //   naming = naming.append(mode+"_");
  // }
  // else {
  //   naming = naming.append(homi+"_");
  // }
  // naming=naming.append(homi+"_");
  naming = naming.append("_" + pgm + "_");
  ...
  if (!ReplayKTestDir.empty() || !ReplayKTestFile.empty()) {
    ...
    for (std::vector<KTest*>::iterator
      ...
      interpreter->runFunctionAsMain(mainFn, out->numArgs, out->args, pEnvp, naming, dirname, trial, homiflag, iterIndex);
    ...
    std::vector<std::string> queryPathVec;
    for (std::vector<std::string>::iterator
    ...
    for (std::vector<std::string>::iterator
        it = KqueryFiles.begin(), ie = KqueryFiles.end();
        it != ie; ++it) {
      queryPathVec.push_back(it->c_str());
    }

    if (!queryPathVec.empty()) {
      klee_message("KLEE: using %lu kquery paths for pruning algorithm\n", queryPathVec.size());
      interpreter->useQueryPath(&queryPathVec);
    }
    ...
    interpreter->runFunctionAsMain(mainFn, pArgc, pArgv, pEnvp, naming, dirname, trial, homiflag, iterIndex);
