...
cl::opt<std::string> SymMode(
    "symmode",
    cl::desc("only symtuner or with homi"),
    cl::init("mode"),
    cl::cat(PruningStates));

cl::opt<std::string> iterIndex(
    "iterIndex",
    cl::desc("iterIndex"),
    cl::init("0"),
    cl::cat(PruningStates));
...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    : Interpreter(opts), interpreterHandler(ih), searcher(0),
      ...
      replayKTest(0), replayPath(0), usingSeeds(0), usingQueryPath(0),
...
// ================================ For Seed Pruning Algorithm
std::vector<std::string> Executor::readLinesFromFile(const std::string& filename) {
...
// Split a string into newline-separated entries
std::vector<std::string> Executor::splitStringByNewline(const std::string& input) {
  std::vector<std::string> lines;

  size_t start = input.find("(query [");
  start += 8; // Start after the "(query [" prefix

  size_t end = input.rfind("] false)");

  std::string middle = input.substr(start, end - start);
...
bool Executor::isSubsequence(const std::vector<std::string>& big, const std::vector<std::string>& small) {
  if (small.empty()) {
    return true;  // An empty subsequence is always contained.
  }

  auto it_big = big.begin();
  auto it_small = small.begin();

  while (it_big != big.end()) {
      if (*it_big == *it_small) {
          ++it_small;
          if (it_small == small.end()) {
            return true; // All entries in small were found.
          }
      }
      ++it_big;
  }
  return false; // Some entries in small were not found.
}
// ================================ For Seed Pruning Algorithm

/*************************************Homi**************************************/
std::map<std::string, unsigned> Executor::read_feature(std::string dirname, std::string naming, bool homiflag, std::string iterIndex) {
  std::map<std::string, unsigned> Feature_id; 
  Feature_id.clear();

  std::string f_name = "";
  std::string re_naming;
  std::cout << "NNN : " << naming << std::endl;

  size_t pos_auto = naming.find("auto");
  size_t pos_roundrobin = naming.find("roundrobin");

  if (pos_auto != std::string::npos) {
    re_naming = naming;
    re_naming.replace(pos_auto, 4, "featmaker");
  } 
  
  else if (pos_roundrobin != std::string::npos) {
    re_naming = naming;
    re_naming.replace(pos_roundrobin, 10, "featmaker");
  }
  ...
  // std::string re_naming = naming.replace(naming.find("auto"), 4, "featmaker");
  f_name = dirname + "/" + re_naming + "tc_dir/weights/" + iterIndex + "feature_data";

  feature_id = 0; 
  std::cout << "F_NAME : " << f_name << "\n";
...
std::vector<unsigned> Executor::read_pruningratio(std::string dirname, std::string naming, bool homiflag, std::string iterIndex){
  ...
  // if (homiflag) {
  //   std::string re_naming = naming.replace(naming.find("auto"), 4, "featmaker");
  //   p_name = dirname + "/" + re_naming + "tc_dir/weights/" + iterIndex + "pruning_ratio";
  // }

  // else {
  //   p_name = dirname + "/" + naming + "tc_dir/weights/" + iterIndex + "pruning_ratio";
  // }
...
std::vector<double> read_wvector(std::string dirname, std::string naming, std::string &wv_name, bool homiflag){
    ...
    std::string w_file = "";
    // if (homiflag) {
    //   std::string re_naming = naming.replace(naming.find("auto"), 8, "featmaker");
    //   w_file = dirname + "/" + re_naming + "tc_dir/" + dir + "/" + wv_name;
    // }

    // else {
    //   w_file = dirname + "/" + naming + "tc_dir/" + dir + "/" + wv_name;
    // }
    std::string re_naming;
    size_t pos_auto = naming.find("auto");
    size_t pos_roundrobin = naming.find("roundrobin");
    ...
    std::cout << "N_NAME : " << naming << "\n";
    // std::string re_naming = naming.replace(naming.find("auto"), 4, "featmaker");
    w_file = dirname + "/" + re_naming + "tc_dir/" + dir + "/" + wv_name;

    std::cout << "W_File : " << w_file << "\n";
...
void Executor::run(ExecutionState &initialState, std::string naming, std::string dirname, std::string trial, bool homiflag, std::string iterIndex) {
// void Executor::run(ExecutionState &initialState) {
  ...
  if (kqueryVec.size() != 0) {
    ...
    for (const auto &each_state : copy_states) {
      std::string last_string;
      std::string res;
      llvm::raw_string_ostream info(last_string);    
      ExprPPrinter::printConstraints(info, each_state->constraints);
      res = info.str(); 

      std::vector<std::string> bcVec = splitStringByNewline(res);

      for (std::vector<std::vector<std::string>>::iterator iter1 = kqueryVec.begin(); iter1 != kqueryVec.end(); iter1++) {
        if (isSubsequence(*iter1, bcVec)) {
          ExecutionState *es = each_state;
          states.erase(es);
          processTree->remove(es->ptreeNode);
          break;
        }
  ...
  klee_message("After Pruning, the size of states : %d", (int) states.size());
  ...
  if(Homi || homiflag){ 
    ...
    feat_id_map_ = read_feature(dirname, naming, homiflag, iterIndex);
    pratio_vec_ = read_pruningratio(dirname, naming, homiflag, iterIndex);
  }
  ...
  while (!states.empty() && !haltExecution) {
    ...
    if (elapsed_time >= prev_time){
      ...
      if((Homi || homiflag) && prev_time>=time_cycle && states.size()>500){
        std::vector<std::vector<int>> fv_vecs;
        //// Obtain the weight vector for the state-pruning. ////
        
        std::string wv_name = "";
        if (homiflag) {
          wv_name = trial + "trials/" + std::to_string(trial_idx + 1) + ".w";
        }

        else {
          wv_name = iterIndex + "trials/" + std::to_string(trial_idx + 1) + ".w";
        }

        wvector_ = read_wvector(dirname, naming, wv_name, homiflag);
...
void Executor::runFunctionAsMain(Function *f,
  ...
  // ===== Initialization for Seeding Pruning Algorithm
  if (usingQueryPath) {
    for (std::vector<std::string>::const_iterator 
          it = usingQueryPath->begin(), ie = usingQueryPath->end();
          it != ie; ++it) {
      kqueryVec.push_back(readLinesFromFile(*it));
    }
  }
  ...
  run(*state, naming, dirname, trial, homiflag, iterIndex);
  processTree = nullptr;
