...
if __name__=="__main__":
    ...
    fg_homi = feature_generator.feature_generator({}, top_dir, arguments)
    ...
    time_budget_handler_1 = TimeBudgetHandler(arguments.total_budget // 4, arguments.minimum_time_portion, arguments.step, arguments.increase_ratio, arguments.minimum_time_budget)
    ...
    mode = "symtuner"
    time_budget_handler_2 = TimeBudgetHandler(arguments.total_budget // 4, arguments.minimum_time_portion, arguments.step, arguments.increase_ratio, arguments.minimum_time_budget)
    ...
    mode = "homi"
    time_budget_handler_3 = TimeBudgetHandler(arguments.total_budget // 4, arguments.minimum_time_portion, arguments.step, arguments.increase_ratio, arguments.minimum_time_budget)
    for i, _ in enumerate(time_budget_handler_3):
        weight_idx = i % arguments.n_scores
        policy = 'explore' if i < arguments.exploration_steps else None
        parameters = symtuner.sample(policy=policy)
        parameters['-output-dir'] = f"{top_dir}/result/iteration-{feat_iter}/{weight_idx}"
        parameters['-max-time'] = 120

        stgy = "featmaker"
        d_name = "All"
        Run_KLEE(pconfig, pgm, stgy, arguments.total_budget, arguments.core_num, homi_iter,
                 mode, d_name, homi_step, None, top_dir, feat_iter, weight_idx,
                 parameters=parameters, flag="rand", ram=False)
        dir_name, testcases_homi, homi_rm_cmd = Run_gcov(
            pconfig, pgm, stgy, homi_iter, mode, arguments.core_num, homi_data, d_name)
        os.chdir(cur_dir)
        symtuner.add(pconfig, testcases_homi, parameters=parameters,
                     evaluation_kwargs=evaluation_argument, rm_cmd=homi_rm_cmd, flag="basehomi")

        for key, value in symtuner.temphomidata.items():
            symtuner.featmakerdata[weight_idx][key] = value
        homi_data = symtuner.homidata
        topk_testcases = SetCoverProblem(homi_data, homi_iter)
        fg_homi.n_scores = len(symtuner.homidata)
        fg_homi.collect_homi(homi_iter + 1, homidata=symtuner.homidata)
        fg_homi.extract_feature_homi()
        PruningStgy_Generator_Random(
            pconfig, pgm, stgy, arguments.core_num, fg_homi.data["features"],
            dir_name, topk_testcases, homi_iter, mode)
    ...
    mode = "ram"
    time_budget_handler_4 = TimeBudgetHandler(arguments.total_budget // 4, arguments.minimum_time_portion, arguments.step, arguments.increase_ratio, arguments.minimum_time_budget)
    for i, _ in enumerate(time_budget_handler_4):
        ram_step = i
    ...
    coverage, bugs = symtuner.get_coverage_and_bugs()
    get_logger().info(f'Sequence-based integration of SymTuner+FeatMaker+HOMI+RAM achieved {len(coverage)} coverage.')
