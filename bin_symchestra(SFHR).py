...
if __name__=="__main__":
    ...
    datastructure = {"symtuner" : dict(), "featmaker" : dict(), "homi" : dict(), "ram" : dict()}
    querystructure = set()
    usedBuffer = list()
    homi_data = {}
    ...
    for i, _ in enumerate(time_budget_handler):
        if mode == "symtuner":
            ...
            weight_idx = weight_idx % arguments.n_scores
            policy = 'explore'
            parameters = symtuner.sample(policy=policy)
            ...
            for testcase, branches in symtuner.homidata.items():
                if filterSeeds.checkAndUpdateQueries(testcase, querystructure):
                    datastructure[mode][testcase] = branches
        ...
        get_logger().info(f'Execution Number: {i + 1} '
                          ...
                          f'All Coverage: {len(allCoverage)} '
                          f'Steps: {sym_step} {feat_step} {homi_step} {ram_step} '
                          f'Count: {count} '
                          f'execCount : {execCount}')
    ...
    coverage, bugs = symtuner.get_coverage_and_bugs()
    get_logger().info(f'SymChestra-based integration of SymTuner+FeatMaker+HOMI+RAM achieved {len(coverage)} coverage.')
