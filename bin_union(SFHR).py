...
from homi_func import PruningStgy_Generator_Random
...
if __name__=="__main__":
    ...
    argparser.add_argument('core_num', nargs='?', default='1')
    ...
    symtuner_args = argparser.parse_args()

    if symtuner_args.pgm is None:
        print("Required option is empty: pgm")
        exit(1)
        
    if symtuner_args.output_dir is None:
        print("Required option is empty: output_dir")
        exit(1)
            
    pgm = symtuner_args.pgm
    output_dir = symtuner_args.output_dir
    ...
    # Symtuner Setting
    if symtuner_args.debug:
        get_logger().setLevel('DEBUG')

    if symtuner_args.generate_search_space_json:
    ...
    symtuner = KLEESymTuner('/root/symchestra/klee_featmaker/build/bin/klee-replay', 'gcov', 10, symtuner_args.search_space, symtuner_args.exploit_portion)
    evaluation_argument = {'folder_depth': symtuner_args.gcov_depth}
    ...
    fg = feature_generator.feature_generator(data, top_dir, symtuner_args)
    fg_homi = feature_generator.feature_generator({}, top_dir, symtuner_args)

    if symtuner_args.main_option == "featmaker":
        wg = weight_generator.learning_weight_generator(data, top_dir, symtuner_args.n_scores)
    else:
        wg = weight_generator.random_weight_generator(data, top_dir, symtuner_args.n_scores)
    ke = klee_executor.klee_executor(pconfig, top_dir, symtuner_args, symtuner, wg)
    ...
    time_budget_handler = TimeBudgetHandler(symtuner_args.total_budget, symtuner_args.minimum_time_portion, symtuner_args.step, symtuner_args.increase_ratio, symtuner_args.minimum_time_budget)
    ...
    while firstTrain:
        ...
        beforeCoverage = symtuner.allCoverage
        allCoverage |= beforeCoverage
        execCount += 1
        ...
        get_logger().info(f'Execution Number: {execution_num + 1} '
                          ...
                          f'Coverage: {len(beforeCoverage)} '
                          f'All Coverage: {len(allCoverage)} '
                          f'Steps: {step} '
                          f'Count: {count} '
                          f'execCount : {execCount}')

        if execCount % symtuner_args.n_scores == 0 and execCount != 0:
            firstTrain = False
            count = 0
            execCount = 0
    ...
    for i, _ in enumerate(time_budget_handler):
        weight_idx = i % symtuner_args.n_scores
        policy = 'explore' if step < symtuner_args.exploration_steps else None
        parameters = symtuner.sample(policy=policy)

        parameters['-output-dir'] = f"{top_dir}/result/symtuner/{step}"
        parameters['-max-time'] = max_time
        
        stgy = "featmaker"
        d_name = "All"
        Run_KLEE(pconfig, pgm, stgy, symtuner_args.total_budget, symtuner_args.core_num, homi_iter,
                 "homi", d_name, homi_step, None, top_dir, feat_iter, weight_idx,
                 parameters=parameters, flag="rand", ram=True)
        dir_name, testcases_symtuner, homi_rm_cmd = Run_gcov(
            pconfig, pgm, stgy, homi_iter, "homi", symtuner_args.core_num, homi_data, d_name)
        os.chdir(cur_dir)
        symtuner.add(pconfig, testcases_symtuner, parameters=parameters,
                     evaluation_kwargs=evaluation_argument, rm_cmd=homi_rm_cmd, flag="else")
        ...
        fg_homi.n_scores = len(homi_data)
        fg_homi.collect_homi(homi_iter + 1, homidata=homi_data)
        fg_homi.extract_feature_homi()
        PruningStgy_Generator_Random(
            pconfig, pgm, stgy, symtuner_args.core_num, fg_homi.data["features"],
            dir_name, topk_testcases, homi_iter, "homi")
        ...
        allCoverage = coverage | symtuner.tempfeatmakerCoverage | symtuner.tempcombinationCoverage
        execCount += 1
    ...
    coverage, bugs = symtuner.get_coverage_and_bugs()
    get_logger().info(f'Union-based integration of SymTuner+FeatMaker+HOMI+RAM achieved {len(coverage)} coverage.')
